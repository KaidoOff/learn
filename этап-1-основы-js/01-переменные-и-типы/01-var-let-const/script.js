...
/*
Переменные — это контейнеры для хранения данных, к которым можно обращаться и изменять в процессе работы программы.
Они могут хранить любые типы данных.

var, let, const — это ключевые слова для объявления переменной.

var — устаревший способ объявления переменных. 
  • Подвержен hoisting’у (объявление поднимается вверх области видимости).
  • Можно переобъявить в одной области видимости.
  • Имеет функциональную или глобальную область видимости, но не ограничен блоком 
    (например, объявленный внутри if будет доступен и снаружи).

let — современный способ объявления переменной. 
  • Ограничен блочной областью видимости.
  • Можно изменять значение, но нельзя повторно объявить в одной области.

const — ключевое слово для объявления переменной, значение которой нельзя переназначить.
  • Также ограничена блочной областью видимости.
  • Для примитивов значение фиксировано, но у объектов и массивов можно изменять содержимое.
*/
